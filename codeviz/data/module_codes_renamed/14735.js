...
const o = require("Environment-Dependent-Module")("helix-fetch");
const M_AbortSignalManager_maybe = require("AbortSignalManager");
const { Body: a } = require("BodyStreamParser");
const { Headers: c } = require("HTTP-Header-Validator");
const { Request: l } = require("RequestHandler");
const { Response: u } = require("ResponseManager");
const {
  FetchBaseError: d,
  FetchError: p,
  AbortError: h,
} = require("FetchErrorHandler");
const {
  AbortController: f,
  AbortSignal: g,
  TimeoutSignal: m,
} = require("AbortSignalManager");
const M_CachePolicyManager_maybe = require("CachePolicyManager");
const { cacheableResponse: v } = require("CacheableResponseModule");
const { sizeof: _ } = require("Object-Size-Calculator");
const { isFormData: b } = require("FormDataSerializerModule");
const { context: w, RequestAbortedError: C } = require("HelixFetchCoreAPI");
const E = ["GET", "HEAD"];
const T = "push";
const S = async (e, t, r) => {
...
const x = async (e, t, r) => {
  ...
  const n = new M_CachePolicyManager_maybe(t, r, {
    shared: false,
  });
  if (n.storable()) {
    const i = await v(r);
...
class A {
  constructor(e) {
    ...
    this.cache = new M_AbortSignalManager_maybe({
      max: i,
      maxSize: r,
