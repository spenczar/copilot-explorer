...
const M_ImportContextParser_maybe = require("ImportContextParser");
const M_NeighboringSnippetManager_maybe = require("NeighboringSnippetManager");
const M_sibling_function_manager_maybe = require("sibling-function-manager");
const M_TokenizerManager_maybe = require("TokenizerManager");
const M_prompt_background_manager_maybe = require("prompt-background-manager");
const M_EditDistanceCalculator_maybe = require("EditDistanceCalculator");
...
class PromptOptions {
  constructor(e, r) {
    ...
    this.neighboringTabs =
      M_NeighboringSnippetManager_maybe.NeighboringTabsOption.Eager;
    this.neighboringSnippetTypes =
      M_NeighboringSnippetManager_maybe.NeighboringSnippetType.NeighboringSnippets;
    this.lineEnding = v.ConvertToUnix;
    this.suffixPercent = 0;
    this.suffixStartMode = b.Cursor;
    this.tokenizerName = M_TokenizerManager_maybe.TokenizerName.cushman001;
...
exports.getPrompt = async function (e, r, m = {}, v = [], w = []) {
  ...
  const x = M_TokenizerManager_maybe.getTokenizer(E.tokenizerName);
  let k = false;
  ...
  const P = new M_prompt_background_manager_maybe.Priorities();
  const R = P.justBelow(M_prompt_background_manager_maybe.Priorities.TOP);
  const N =
    E.languageMarker == p.Always
      ? P.justBelow(M_prompt_background_manager_maybe.Priorities.TOP)
      : P.justBelow(R);
  const O =
  ...
  const B = new M_prompt_background_manager_maybe.PromptWishlist(
    x,
    E.lineEnding
  );
  let F;
  ...
  if (E.languageMarker != p.NoMarker) {
    const e = newLineEnded(
      M_language_marker_constants_maybe.getLanguageMarker(r)
    );
    F = B.append(
      e,
      M_prompt_background_manager_maybe.PromptElementKind.LanguageMarker,
      N
    );
  }
  if (E.pathMarker != h.NoMarker) {
    const e = newLineEnded(M_language_marker_constants_maybe.getPathMarker(r));
    if (e.length > 0) {
      j = B.append(
        e,
        M_prompt_background_manager_maybe.PromptElementKind.PathMarker,
        O
      );
    }
  }
  if (E.localImportContext != y.NoContext)
    for (const e of await M_ImportContextParser_maybe.extractLocalImportContext(
      r,
      E.fs
    ))
      B.append(
        newLineEnded(e),
        M_prompt_background_manager_maybe.PromptElementKind.ImportedFile,
        D
      );
  const U =
    E.neighboringTabs ==
      M_NeighboringSnippetManager_maybe.NeighboringTabsOption.None ||
    0 == v.length
      ? []
      : await M_NeighboringSnippetManager_maybe.getNeighborSnippets(
          r,
          v,
  ...
  function $() {
    w.map(M_SnippetNormalizerManager_maybe.normalizeSnippetScore)
      .sort((e, t) => e.score - t.score)
      .slice(-E.numberOfSnippets)
      .map((e) => ({
        score: e.score,
        snippet: M_SnippetNormalizerManager_maybe.announceSnippet(e)
          .map(
            (e) =>
              M_language_marker_constants_maybe.comment(e, r.languageId) + "\n"
          )
          .join(""),
      ...
      .forEach((e) => {
        B.append(
          e.snippet,
          M_prompt_background_manager_maybe.PromptElementKind.SimilarFile,
          M,
  ...
  if (E.includeSiblingFunctions == f.NoSiblings) H = I.substring(0, A);
    ...
    const {
      ...
      beforeInsertion: t,
      afterInsertion: n,
    } = await M_sibling_function_manager_maybe.getSiblingFunctions(r);
    B.appendLineForLine(
      t,
      M_prompt_background_manager_maybe.PromptElementKind.BeforeCursor,
      R
    ).forEach((e) => q.push(e));
    let i = L;
    e.forEach((e) => {
      B.append(
        e,
        M_prompt_background_manager_maybe.PromptElementKind.AfterCursor,
        i
      );
      i = P.justBelow(i);
    });
    ...
    return B.fulfill(E.maxPromptLength);
  {
    ...
    if (
      ...
      e = await M_sibling_function_manager_maybe.getSiblingFunctionStart(r);
    }
    ...
    if (E.suffixMatchCriteria == _.Equal)
      ...
      const e = (0, M_EditDistanceCalculator_maybe.findEditDistanceScore)(
