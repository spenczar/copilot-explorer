...
const M_GitUrlParser_maybe = require("GitUrlParser");
...
exports.extractRepoInfoInBackground = function (e, t) {
  if (!t) return;
  const r = M_path.dirname(t);
  return l(e, r);
};
const l = (function (e, t) {
  const r = new M_LRUCacheManager_maybe.LRUCache(1e4);
  const n = new Set();
  return (t, ...i) => {
...
async function u(e, t) {
  const r = await (async function (e, t) {
    ...
    const i = e.get(M_TreeNodeUtils_maybe.FileSystem);
  ...
  if (!r) return;
  const i = e.get(M_TreeNodeUtils_maybe.FileSystem);
  const s = M_path.join(r, ".git", "config");
