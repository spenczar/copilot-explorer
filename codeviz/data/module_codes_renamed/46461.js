...
a.E = s.emailAddress;
a.emailAddress = "E";
var c = M_OptionsManager_maybe.pki.rsa.publicKeyValidator;
var l = {
  name: "Certificate",
...
var g = function (e) {
  switch (s[e.signatureOid]) {
    ...
    case "sha1WithRSASignature":
      return M_OptionsManager_maybe.md.sha1.create();
    case "md5WithRSAEncryption":
      return M_OptionsManager_maybe.md.md5.create();
    case "sha256WithRSAEncryption":
    case "RSASSA-PSS":
      return M_OptionsManager_maybe.md.sha256.create();
    case "sha384WithRSAEncryption":
      return M_OptionsManager_maybe.md.sha384.create();
    case "sha512WithRSAEncryption":
      return M_OptionsManager_maybe.md.sha512.create();
    default:
      var t = new Error(
  ...
  switch (r.signatureOid) {
    ...
    case s["RSASSA-PSS"]:
      ...
      if (
        ...
        undefined === M_OptionsManager_maybe.md[i]
      )
        throw (
      ...
      if (
        ...
        undefined === M_OptionsManager_maybe.mgf[o]
      )
        throw (
      ...
      o = M_OptionsManager_maybe.mgf[o].create(
        M_OptionsManager_maybe.md[i].create()
      );
      if (
        void 0 === (i = s[r.signatureParameters.hash.algorithmOid]) ||
        void 0 === M_OptionsManager_maybe.md[i]
      )
        throw (
      ...
      t = M_OptionsManager_maybe.pss.create(
        M_OptionsManager_maybe.md[i].create(),
        o,
...
function y(e) {
  for (
    ...
    if ("valueTagClass" in t && (l = t.valueTagClass) === i.Type.UTF8) {
      c = M_OptionsManager_maybe.util.encodeUtf8(c);
    }
...
function _(e, t) {
  ...
  if ("keyUsage" === e.name) {
    ...
    for (var p = 0; p < e.altNames.length; ++p) {
      ...
      if (7 === v.type && v.ip) {
        if (null === (l = M_OptionsManager_maybe.util.bytesFromIP(v.ip)))
          throw (
            (((w = new Error(
    ...
    if (e.serialNumber) {
      var m = M_OptionsManager_maybe.util.hexToBytes(
...
function w(e) {
  ...
  for (r = e.attributes, o = 0, undefined; o < r.length; ++o) {
    ...
    if (c === i.Type.UTF8) {
      a = M_OptionsManager_maybe.util.encodeUtf8(a);
    }
    var l = false;
...
o.certificateFromPem = function (e, t, r) {
  var s = M_OptionsManager_maybe.pem.decode(e)[0];
  if (
    "CERTIFICATE" !== s.type &&
...
o.getPublicKeyFingerprint = function (e, t) {
  var r;
  var s = (t = t || {}).md || M_OptionsManager_maybe.md.sha1.create();
...
o.createCertificate = function () {
  ...
  e.sign = function (t, r) {
    e.md = r || M_OptionsManager_maybe.md.sha1.create();
  ...
  e.verifySubjectKeyIdentifier = function () {
    for (
      ...
      if (i.id === t) {
        ...
        return (
          M_OptionsManager_maybe.util.hexToBytes(i.subjectKeyIdentifier) === o
        );
      }
    }
    return false;
...
o.certificateFromAsn1 = function (e, t) {
  ...
  var u = M_OptionsManager_maybe.util.createBuffer(r.certSerialNumber);
  c.serialNumber = u.toHex();
  c.signatureOid = M_OptionsManager_maybe.asn1.derToOid(r.certSignatureOid);
  c.signatureParameters = f(c.signatureOid, r.certSignatureParams, true);
  c.siginfo.algorithmOid = M_OptionsManager_maybe.asn1.derToOid(
  ...
  var m = M_OptionsManager_maybe.md.sha1.create();
  ...
  var _ = M_OptionsManager_maybe.md.sha1.create();
  ...
  if (t.id in s)
    if (((t.name = s[t.id]), "keyUsage" === t.name)) {
      ...
      for (var p = 0; p < c.value.length; ++p) {
        ...
        switch ((t.altNames.push(h), d.type)) {
          ...
          case 6:
            break;
          case 7:
            h.ip = M_OptionsManager_maybe.util.bytesToIP(d.value);
            break;
          case 8:
      ...
      "subjectKeyIdentifier" === t.name &&
        ((c = i.fromDer(t.value)),
        (t.subjectKeyIdentifier = M_OptionsManager_maybe.util.bytesToHex(
          c.value
        )));
  return t;
};
o.certificationRequestFromAsn1 = function (e, t) {
  ...
  c.signatureOid = M_OptionsManager_maybe.asn1.derToOid(r.csrSignatureOid);
  ...
  var u = M_OptionsManager_maybe.md.sha1.create();
  c.subject.getField = function (e) {
    return h(c.subject, e);
  };
  c.subject.addField = function (e) {
...
o.createCaStore = function (e) {
  ...
  function s(e) {
    if (!e.hash) {
      var t = M_OptionsManager_maybe.md.sha1.create();
  ...
  t.addCertificate = function (e) {
    if ("string" == typeof e) {
      e = M_OptionsManager_maybe.pki.certificateFromPem(e);
    }
    s(e.subject);
    if (!t.hasCertificate(e))
      if (e.subject.hash in t.certs) {
        var r = t.certs[e.subject.hash];
        M_OptionsManager_maybe.util.isArray(r) || (r = [r]),
          r.push(e),
  ...
  t.hasCertificate = function (e) {
    ...
    var t = r(e.subject);
    if (!t) return false;
    if (M_OptionsManager_maybe.util.isArray(t)) {
      t = [t];
    }
    for (
    ...
    for (var r in t.certs)
      if (t.certs.hasOwnProperty(r)) {
        ...
        if (M_OptionsManager_maybe.util.isArray(i))
          for (var o = 0; o < i.length; ++o) e.push(i[o]);
        else e.push(i);
      }
    return e;
  };
  t.removeCertificate = function (e) {
    ...
    var c = r(e.subject);
    if (!M_OptionsManager_maybe.util.isArray(c)) {
  ...
  do {
    ...
    if (null === c) {
      if (
        ...
        var h = d;
        M_OptionsManager_maybe.util.isArray(h) || (h = [h]);
    ...
    if (true !== w)
      throw (
        ...
        (w || 0 === w) &&
          ("object" != typeof w || M_OptionsManager_maybe.util.isArray(w)
