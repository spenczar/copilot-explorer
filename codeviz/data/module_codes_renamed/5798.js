...
class Logger {
  ...
  log(e, t, r, ...i) {
    ...
    const d = e.get(M_ClockModule_maybe.Clock).now().toISOString();
    const p = `[${a}] [${this.context}] [${d}]`;
    c.logIt(e, t, p, ...i);
  }
  shouldLog(e, t, r) {
    if (verboseLogging(e)) return true;
    const n = M_editor_config_constants_maybe.getConfig(
      e,
      M_editor_config_constants_maybe.ConfigKey.DebugFilterLogCategories
    );
    if (n.length > 0 && !n.includes(r)) return false;
    if (M_editor_config_constants_maybe.isProduction(e))
      return t >= this.minLoggedLevel;
    const o = M_editor_config_constants_maybe.getConfig(
      e,
      M_editor_config_constants_maybe.ConfigKey.DebugOverrideLogLevels
    );
    return (
      t >=
