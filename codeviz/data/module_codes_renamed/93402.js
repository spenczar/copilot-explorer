...
exports.postInsertionTasks =
  ...
  exports.postInsertConfiguration =
    undefined;
const M_TextDocumentChangeTracker_maybe = require("TextDocumentChangeTracker");
...
const h = new M_LoggingUtils_maybe.Logger(
...
async function captureCode(e, t, r) {
  ...
  const l = await M_prompt_parser_utils_maybe.extractPrompt(e, n, c);
  const u =
    "prompt" === l.type
      ? l.prompt
      : {
  ...
  const f = M_language_indentation_utils_maybe.contextIndentationFromText(
    o,
    r,
    n.languageId
  );
  const g = M_language_indentation_utils_maybe.indentationBlockFinished(
    f,
    undefined
  );
  const m = await g(d);
...
function m(e, t, r, n) {
  ...
  const o = M_editDistanceCalculator_maybe.lexEditDistance(i, t);
  const s = o.lexDistance / o.needleLexLength;
  const { distance: a } = M_editDistanceCalculator_maybe.editDistance(
...
exports.postRejectionTasks = function (e, t, r, o, s) {
  s.forEach(({ completionText: r, completionTelemetryData: n }) => {
    ...
    M_ghost_text_telemetry_utils_maybe.telemetryRejected(e, t, n);
  });
  const a = new M_TextDocumentChangeTracker_maybe.ChangeTracker(e, o, r);
...
exports.postInsertionTasks = async function (e, r, o, s, a, c) {
  ...
  M_ghost_text_telemetry_utils_maybe.telemetryAccepted(e, r, c);
  const y = o.trim();
  const v = new M_TextDocumentChangeTracker_maybe.ChangeTracker(e, a, s);
  const _ = async (t) => {
    await (async function (e, t, r, n, i, o, s, a) {
      ...
      if (c) {
        ...
        const p = s
          ...
          .extendedBy({}, d);
        M_TelemetryReporterModule_maybe.telemetry(e, t + ".stillInCode", p);
        if (o.captureCode) {
          const {
            prompt: r,
            capturedCode: c,
  ...
  if (
    ...
    M_RuntimeModeManager_maybe.isRunningInTest(e)
  ) {
    _({
      seconds: 0,
      captureCode: false,
  ...
  e.get(M_PostInsertionNotifierModule_maybe.PostInsertionNotifier).emit(
    "onPostInsertion",
    {
      ctx: e,
      insertionCategory: r,
